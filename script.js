...
class VoXtraApp {
    ...
    setupEventListeners() {
        ...
        if (libraryBtn) {
            libraryBtn.addEventListener("click", () => {
                ...
                this.renderProjectsLibrary(); // Обновляем библиотеку при переходе
            });
        }
    ...
    <div class="project-icon">${trackInfo.icon}</div>
                    <div class="project-info">
                        ...
                        <div class="project-meta">
                            ...
                            <span>🎵 ${
                                this.getGenreName(project.genre) || "Без жанра"
                            }</span>
    ...
    async generateWithAI(prompt, instrument, style, tempo) {
        try {
            ...
            // Если не сработало, пробуем внешние API
            if (!audioUrl) {
                audioUrl = await this.tryExternalAPIs(enhancedPrompt);
            }

            // Если всё ещё нет, используем демо-трек
            if (!audioUrl) {
                this.showToast(
                    "⚠️ Используем демо-трек (AI временно недоступен)",
                    "warning"
                );
                audioUrl = this.getDemoAudioUrl(this.currentTrackType);
            }
    ...
    async tryExternalAPIs(prompt) {
        // Здесь можно добавить вызовы к различным AI API
        // Например: Hugging Face, Replicate, OpenAI и т.д.

        // Заглушка - в реальном проекте здесь будут настоящие API вызовы
        return null;
    }
